...
let ComponentsAdapter = function(configuration) {
    ...
    glob(location, (err, files) => {
        ...
        async.waterfall(helperTasks, function finish() {
            dev.debug('[%s]'.yellow + ' => components loaded with success'.green, helperTasks.length);
            configuration.doNext(null);
        });
    });
};

module.exports = ComponentsAdapter;